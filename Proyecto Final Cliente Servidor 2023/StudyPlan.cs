...
{
    public partial class FmStudyPlan : Form
    {
        ...
        private void MostrarDatos()
        {
            ...
            string consulta = "SELECT * FROM StudyPlan Where status= 1";
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                dgvStudyPlan.DataSource = dataTable;
            }
        }
        ...
        private void btnAgregar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(txtNameStudyPlan.Text) || string.IsNullOrEmpty(txtNameStudyPlan.Text))
                {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string query = "INSERT INTO StudyPlan (nameStudyPlan,code,duration,credits,idCareer,status)" +
                            "VALUES (@nameStudyPlan, @code,@duration,@credits,@idCareer, @status)";

                    DateTime Duration = dtpDuration.Value;


                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        // Agregar los parámetros para la inserción
                        command.Parameters.AddWithValue("@nameStudyPlan", txtNameStudyPlan.Text);
                        command.Parameters.AddWithValue("@code", txtCode.Text);
                        command.Parameters.AddWithValue("@duration", Duration);
                        command.Parameters.AddWithValue("@credits", txtCredits.Text);
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            try
            {
                if (dgvStudyPlan.SelectedRows.Count > 0)
                {
                    int idToDelete = Convert.ToInt32(dgvStudyPlan.SelectedRows[0].Cells["idStudyPlan"].Value);

                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        connection.Open();

                        string query = "UPDATE StudyPlan SET status = 0 WHERE idStudyPlan = @IdParaActualizar";
