...
namespace Proyecto_Final_Cliente_Servidor_2023
{
    ...
    public partial class FmClass : Form
    {
        ...
        private void MostrarDatos()
        {
            ...
            string consulta = "SELECT * FROM Class Where status=1";
        ...
        private void btnAgregar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(txtNameClass.Text) || string.IsNullOrEmpty(txtDescriptionClass.Text))
                {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string query = "INSERT INTO Class (nameClass, descriptionClass,idCourse,status)" +
                                   "VALUES (@nameClass, @descriptionClass, @idCourse, @status)";

                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        // Parámetros de la consulta de inserción
                        command.Parameters.AddWithValue("@nameClass", txtNameClass.Text);
                        command.Parameters.AddWithValue("@descriptionClass", txtDescriptionClass.Text);
        ...
        private void btnModificar_Click(object sender, EventArgs e)
        {
            ...
            string descriptionClass = txtDescriptionClass.Text;
            ...
            try
            {
                ...
                {
                    int idToDelete = Convert.ToInt32(dgvClass.SelectedRows[0].Cells["idClass"].Value);

                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        connection.Open();

                        string query = "UPDATE Class SET nameClass = @NameClass, descriptionClass = @DescriptionClass" +
                          
                          "WHERE idClass = @idClass"; // Reemplaza "TuTabla" por el nombre de tu tabla y ajusta el WHERE según tus necesidades
                        ...
                        using (SqlCommand command = new SqlCommand(query, connection))
                        {
                            ...
                            command.Parameters.AddWithValue("@DescrptionClass", descriptionClass);
                          
                            // Agregar el parámetro para el ID de la fila que se va a actualizar
                            command.Parameters.AddWithValue("@idClass", idToDelete);
            ...
            try
            {
                if (dgvClass.SelectedRows.Count > 0)
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        ...
                        string query = "UPDATE Class SET status = 0 WHERE idClass = @IdParaActualizar";
